...
var gaComplete = function(address, globs, Characteristic){
	...
	if (!globs.config.GroupAddresses[address]) {
		...
		if (Characteristic.props.format === globs.Formats.BOOL) {
			// Boolean
			return new GroupAddress(address, 'automatically generated', DPTTypes.DPT1, true, true, true, 'INITIAL')
		} else if (Characteristic.props.format === globs.Formats.INT || Characteristic.props.format === globs.Formats.UINT8) {
		...
		} else if (Characteristic.props.format === globs.Formats.FLOAT) {
