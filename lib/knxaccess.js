...
allFunctions.knxread = function(groupAddress) {
	...
	if(!(!globs.knxconnection) || globs.knxconnection==='knxjs') {
		...
		knxdConnection.socketRemote({
			...
			knxdConnection.openTGroup(dest, 1, function(err) {
				if (err) {
					globs.errorlog("[ERROR] knxread:openTGroup for " + groupAddress + ": " + err);
				} else {
...
allFunctions.knxreadhash = function(groupAddresses) {
	...
	for ( var address in groupAddresses) {
		if (groupAddresses.hasOwnProperty(address)) { // do not use inherited properties
			addresses.push(address.match(/(\d*\/\d*\/\d*)/)[0]); // clean address
		}
	}
	// stagger read requests to avoid overwhelming the KNX gateway/knxd
	var delay = 500; // ms between requests
	addresses.forEach(function(addr, index) {
		setTimeout(function() {
			allFunctions.knxread(addr);
		}, index * delay);
	});
	globs.debug("[knxreadhash] scheduled " + addresses.length + " read requests with " + delay + "ms interval");
};
...
allFunctions.writeValueHK = function(val, chrKNX, type, reverse) {
	...
	globs.debug("knxAccess.writeValueHK("+ val +","+chrKNX.name+","+(type || 'handler')+","+reverse + ")");
	 ...
	 */
	case globs.Formats.BOOL:
		// boolean is good for any trueish expression from value
		globs.debug("BOOL:[" + characteristic.displayName + "]: Received value from KNX handler:" + val + (type ? " of type " + type : ""));
		//characteristic.setValue(val ? (reverse ? 0 : 1) : (reverse ? 1 : 0), undefined, 'fromKNXBus');
		returnValue = val ? (reverse ? 0 : 1) : (reverse ? 1 : 0);
		break;
	case globs.Formats.INT:
		// let fall through
	case globs.Formats.UINT8:
		// let fall through
	case globs.Formats.UINT16:
		// let fall through 
		//fixes #123 
	case globs.Formats.UINT32:
		// fixes #123
		// to an INT we can assume that the min and max values are set, or a list of allowed values is supplied
		globs.debug("INT:[" + characteristic.displayName + "]: Received value from KNX handler:" + val + (type ? " of type " + type : ""));
		...
		if (characteristic.props.minValue == undefined && characteristic.props.maxValue == undefined) {
			...
			if (characteristic.props.unit === globs.Units.PERCENTAGE) {
				if (!(!globs.knxconnection) || globs.knxconnection === 'knxjs') {
					if (type === 'DPT5') {
		...
		break;
	case globs.Formats.FLOAT:
		globs.debug("FLOAT:[" + characteristic.displayName + "]: Received value from KNX handler:" + val + (type ? " of type " + type : ""));
	...
	if (returnValue!==null) {
		...
		// For FLOAT, use epsilon tolerance to handle floating point precision differences
		var valueChanged;
		if (characteristic.props.format === globs.Formats.FLOAT) {
			var epsilon = characteristic.props.minStep ? characteristic.props.minStep / 2 : 0.001;
			valueChanged = (characteristic.value === null || characteristic.value === undefined ||
				Math.abs(characteristic.value - returnValue) >= epsilon);
		} else {
			valueChanged = (characteristic.value !== returnValue);
		}
		if (valueChanged) {
