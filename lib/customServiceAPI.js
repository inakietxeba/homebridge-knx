...
class customServiceAPI {
        ...
        if (myService.handler === 'Default') {
            for (var iCHars = 0; iCHars < myService.myCharacteristics.length; iCHars++) {
                ...
                if (thatChar.name === field) {
                    return thatChar.getHomekitCharacteristic().value;
                }
            }
        }
        else {
    ...
    /**
     * homekitEventCatcher is bound to homebridge onSet events.
     * onSet is only called by HomeKit, not by updateValue() — no context/callback needed.
     *
     * @private
     */
    homekitEventCatcher(characteristicName, value) {
