...
class ServiceKNX {
	...
	loadServiceData (platformAccessory) {
		...
		// try to find it.
		// getServiceById() replaces deprecated getServiceByUUIDAndSubType() in HAP-NodeJS v1+
		/** @type {Service} */
		var getServiceFn = platformAccessory.getServiceById || platformAccessory.getServiceByUUIDAndSubType;
		this.serviceHK = getServiceFn.call(platformAccessory, this.Services[this.config.ServiceType], this.config.subtype);
