...
class CharacteristicKNX {
    ...
    constructor(service, config, globs) {
        ...
        if (this.chr.props.format === this.globs.Formats.UINT8 || this.chr.props.format === this.globs.Formats.INT) {
        ...
        if (this.chr.props.perms.indexOf(this.globs.Perms.PAIRED_WRITE) > -1) {
            if (this.config.Set) {
                ...
                if (this.service.handler === "Default") {
                    ...
                    this.getHomekitCharacteristic().onSet(this.defaultUpdateKNXValue.bind(this));
                }
            }
            if (this.service.handler !== "Default") {
                // the customAPI handler does not need a Set value, can act differently
                // onSet is only triggered by HomeKit, not by updateValue() — no context needed
                this.getHomekitCharacteristic().onSet((value) => {
                    this.service.customServiceAPI.homekitEventCatcher(this.name, value);
                });

            }
        } // writable
        // if it is readable AND we have a listen section in the config
        if (this.chr.props.perms.indexOf(this.globs.Perms.PAIRED_READ) > -1) {
    ...
    getDPT() {
        ...
        if (this.config.DPT) {
            ...
            if (this.chr.props.format === this.globs.Formats.BOOL) {
                // found boolean, assume DPT1
                return "DPT1";
            }
            if (this.chr.props.format === this.globs.Formats.INT) {
                // found INT, assume DPT5, unless props.unit is Units.PERCENTAGE
                if (this.chr.props.unit === this.globs.Units.PERCENTAGE) {
                    return "DPT5.001";
                } else {
                    return "DPT5";
                }
            }
            if (this.chr.props.format === this.globs.Formats.UINT8) {
            ...
            if (this.chr.props.format === this.globs.Formats.PERCENTAGE) {
                // found PERC, assume DPT5.001
                return "DPT5.001";
            }
            if (this.chr.props.format === this.globs.Formats.FLOAT) {
     ...
     * one address or an array of addresses. Uses DPT settings from getDPT(). Registered via onSet().
     * 
     * @param {number} value
     */
    defaultUpdateKNXValue(value) {
        // onSet() is only invoked by HomeKit, not by updateValue() from the KNX bus — no context check needed
        for (var int2 = 0; int2 < this.setGroupAddressList.length; int2++) {
            //iterate(this.setGroupAddressList[int2]); // DEBUG
            knxAccess.writeValueKNX(value, this.setGroupAddressList[int2], undefined);
        }
    }
}
